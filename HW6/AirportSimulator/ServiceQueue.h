		...
		if (the_queue.empty()) {
			...
			if (!landing_queue->the_queue.empty()) {
				...
				*/
				do
				{
					plane -> service_time = my_random.next_int(max_service_time); // Gets a random number between 0 and max_service_time
				} while (!(min_service_time < plane -> service_time)); // Again if plane is not greater than min_service_time
